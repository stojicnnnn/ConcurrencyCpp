...
protected:
    void SetUp() override {
        ...
        file << "Title,Author,ISBN,Rating,Pages\n";
        file << "The C++ Programming Language,Bjarne Stroustrup,978-0321563842,4.5,1368\n";
        file << "Effective Modern C++,Scott Meyers,978-1491903695,4.8,334\n";
        file << "Clean Code,Robert C. Martin,978-0132350884,4.7,464\n";
        file.close();
    }
...
TEST_F(BooksTest, LoadBooksMissingFile) {
    auto books = loadBooks("non_existent_file.csv");
...
TEST_F(BooksTest, SearchByAuthorFound) {
    ...
    EXPECT_EQ(results[0].title, "Effective Modern C++");
}
...
TEST_F(BooksTest, SearchByTitleFoundCaseInsensitive) {
    ...
    EXPECT_EQ(results[1].title, "Effective Modern C++");
}
